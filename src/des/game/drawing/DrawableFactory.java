...
import des.game.base.ObjectPool;
...
public class DrawableFactory extends BaseObject {
    ...
    private DrawableBufferedBitmapPool mBufferedBitmapPool;
    ...
    public DrawableFactory() {
        ...
        mBufferedBitmapPool = new DrawableBufferedBitmapPool(BITMAP_POOL_SIZE);
    ...
    public DrawableBufferedBitmap allocateDrawableBufferedBitmap() {
        return mBufferedBitmapPool.allocate();
    }
...
private class DrawableBufferedBitmapPool extends TObjectPool<DrawableBufferedBitmap> {
        
        public DrawableBufferedBitmapPool(int size) {
            super(size);
        }
        ...
        protected void fill() {
            int size = getSize();
            for (int x = 0; x < size; x++) {
            	DrawableBufferedBitmap entry = new DrawableBufferedBitmap(0, 0, 0);
                entry.setParentPool(this);
                getAvailable().add(entry);
            }
        }
        ...
        @Override
        public void release(Object entry) {
            ((DrawableBufferedBitmap)entry).reset();
            super.release(entry);
        }
        
        @Override
        public DrawableBufferedBitmap allocate() {
        	DrawableBufferedBitmap result = super.allocate();
            ContextParameters params = sSystemRegistry.contextParameters;
            if (result != null && params != null) {
                result.setViewSize(params.gameWidth, params.gameHeight);
            }
