    ...
    public void onDrawFrame(GL10 gl) {
        ...
        synchronized (this) {
            if (mDrawQueue != null && mDrawQueue.getObjects().getCount() > 0) {
                ...
                DrawableBuffer currentBuffer = null;
                int currentBufferIndex = 0;
                
                if(currentBufferIndex < bufferCount){
                	currentBuffer = (DrawableBuffer)buffers[currentBufferIndex];
                	currentBuffer.startDrawing();
                }
                
                for (int i = 0; i < count; i++) {
                    RenderElement element = (RenderElement)objectArray[i];
                    
                    // do we have a buffer right now?
                    if(currentBuffer != null){
                    	int elementPhase = element.phase/RenderSystem.TEXTURE_SORT_BUCKET_SIZE;
                    	
                    	if(currentBuffer.endLayer < elementPhase){
                    		// this buffer is over

                    		currentBuffer.draw(gl);
                    		
                    		// go to the next buffer if there is one
                    		currentBuffer = null;
                    		currentBufferIndex++;
                    		
                    		// iterate through the buffers until we run out or we find one that ends after the current element phase
                    		while(currentBufferIndex < bufferCount && ((DrawableBuffer)buffers[currentBufferIndex]).endLayer < elementPhase){
                    			currentBufferIndex++;
                            }
                    		if(currentBufferIndex < bufferCount){
                    			currentBuffer = (DrawableBuffer)buffers[currentBufferIndex];
